...
if depsonly is not None:
    print("Device %s not found. Attempting to retrieve device repository from PACROM Github (http://github.com/PACROM)." % device)
...
gapps_git = 'https://github.com/PACROM/android_vendor_pixelgapps'
gapps_branch = 'master'

page = 1
while not depsonly:
    request = urllib.request.Request("https://api.github.com/users/PACROM/repos?page=%d" % page)
    ...
    for repository in repositories:
        ...
        if existing_project != None:
            ...
            if existing_project.attrib['revision'] == repository['branch']:
                print('PACROM/%s already exists' % (repo_name))
            else:
...
def add_to_manifest(repositories):
    ...
    for repository in repositories:
        ...
        project = ElementTree.Element("project", attrib = { "path": repo_target,
            "remote": "github", "name": "PACROM/%s" % repo_name, "revision": master_branch })
...
def add_github_to_manifest(repositories):
    try:
        lm = ElementTree.parse(".repo/local_manifests/electric_manifest.xml")
        lm = lm.getroot()
    except:
    ...
    for repository in repositories:
        ...
        project = ElementTree.Element("project", attrib = { "path": repo_target,
            "remote": "github", "name": repo_name, "revision": master_branch })
...
def fetch_pixel_gapps(repo_path):
    gapps_path = repo_path + '/PACROM_' + device + '.mk'

    try:
        with open(gapps_path, 'r') as f:
            for line in f.readlines():
                if 'pixelgapps' in line:
                    print('Fetching project ' + gapps_git.replace("https://github.com/", ""))
                    git("clone", gapps_git, "-b", gapps_branch, gapps_location)
                    add_gitlab_to_manifest([{'repository':gapps_git.replace("https://github.com/", ""),'target_path':gapps_location,'branch':gapps_branch}])
...
def fetch_dependencies(repo_path, fallback_branch = None):
    ...
    dependencies_path = repo_path + '/PACROM.dependencies'
    syncable_repos = []
    verify_repos = []
